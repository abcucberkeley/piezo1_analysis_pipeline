%% Analysis pipeline performed using puncta tracking
...
rootdir = '/home/sayanseal/doi_10_5061_dryad_w6m905qwm__v20240307/three_plane_stack_data/NonBaptaWT/140um/';
...
% Distances of mean position of tracks from masks across all videos
...
% Standard Deviation of the x and y position of the tracks
std_x = [];
std_y = [];
...
for f = 1:numel(csv_filenames)
    ...
    loc = readtable([csv_filepaths{f} filesep csv_filenames{f}], 'VariableNamingRule', 'preserve');

    tracks = unique(loc.("track_number"));

    for n = 1:numel(tracks)
        localization = loc(loc.("track_number") == tracks(n),:); % analysis performed for each unique track
        ...
        dist_lumen = cmbdtm_lumen(round(mean(y)), round(mean(x)))*.108; % convert to microns
        lumen_dist_arr_per_exp = [lumen_dist_arr_per_exp, dist_lumen];
        lumen_dist_arr = [lumen_dist_arr, dist_lumen];

        dist_outer = cmbdtm_outer(round(mean(y)), round(mean(x)))*.108; % convert to microns
...
headers = {'Filename'};
for n=2:length(lumen_cdf_vals_final(1,:))
...
%% CDF plots for the standard deviation of the x and y position of the tracks
% 
% fig = figure(); 
...
% hold on;
% ecdf(std_y);
% xlabel('Standard Deviations (nm)')
% ylabel('Cumulative Frequency');
% legend('Standard deviation of x', 'Standard deviation of y');
% pbaspect([1,1,1]);
...
title('PIEZO1-HaloTag Localizations');
...
%% Density Scatter Plot for each video (103 videos in total)
...
title(h,'Density scatter plots of PIEZO1-HaloTag localizations', 'FontName', 'Arial', 'Units', 'normalized', 'Position', [0.5, 1.05, 0], 'FontWeight', 'Normal');
c = colorbar(h, 'Position', [0.93 0.22 0.01 0.65], 'Ticks', 0:0.5e-3:5.5e-3);
colormap(c,'jet')
clim(h,[0 5.5e-3]);
save_filename = ['Density_Scatter_individual_1-40_' save_str  '.svg'];
saveas(gcf, [resultsdir save_filename]);
...
colormap(c,'jet')
clim(h,[0 5.5e-3]);
save_filename = ['Density_Scatter_individual_41-80_' save_str  '.svg'];
saveas(gcf, [resultsdir save_filename]);

fig = figure();
fig.Position = [1000, 1000, 1020, 1320];
for i = 81:numel(csv_filenames)
    sp = subplot(8, 5, i-80);
...
h.XLabel.Visible = 'on';
h.YLabel.Visible = 'on';
ylh = ylabel(h,'Distance from Outer Edge (\mum)', 'FontName', 'Arial');
ylh.Position(2) = ylh.Position(2) + 0.25;
xlh = xlabel(h,'Distance from Lumen (\mum)', 'FontName', 'Arial');
xlh.Position(2) = xlh.Position(2) + 0.4;
title(h,'Density scatter plots of PIEZO1-HaloTag localizations', 'FontName', 'Arial', 'Units', 'normalized', 'Position', [0.5, 1.05, 0], 'FontWeight', 'Normal');
c = colorbar(h, 'Position',[0.93 0.5 0.01 0.4], 'Ticks', 0:0.5e-3:5.5e-3);
colormap(c,'jet')
clim(h,[0 5.5e-3]);
save_filename = ['Density_Scatter_individual_81-103_' save_str  '.svg'];
saveas(gcf, [resultsdir save_filename]);